...
const Contact = () => {
  ...
  const onSubmit = async (event) => {
    ...
    formData.append("access_key", "eac12cd9-64a0-498f-a52c-ff2dfe376cc9");
    ...
    className='text-center p-6 py-20 lg:px-32 w-full overflow-hidden' id='Contact'>
      ...
      <form onSubmit={onSubmit} className='max-w-2xl mx-auto text-gray-600 pt-8'>
        <div className='flex flex-wrap'>
            ...
            mt-2' type='text' name='Name' placeholder='Your Name ' required/>
            </div>
             <div className='w-full md:w-1/2 text-left md:pl-4'>
                Your Email
            <input className='w-full border border-gray-300 rounded py-3 px-4
            mt-2' type='email' name='Email' placeholder='Your Email' required/>
            </div>
        </div>
        <div className='my-6 text-left'>
         ...
         <button className='bg-blue-600 text-white py-2 px-12 mb-10 rounded cursor-pointer hover:animate-pulse'>{result ? result : "Send Message"}</button>
      </form>
    </motion.div>
  )
}
